...
class Matrix4
{
   double elements[16]; // layout is row-major
...
public:
   ...
   {
      return makeXRotation(std::cos(ang * CS175_PI / 180), std::sin(ang 
      * CS175_PI / 180));
   }

   static Matrix4 makeYRotation(const double ang)
   {
      return makeYRotation(std::cos(ang * CS175_PI / 180), std::sin(ang 
      * CS175_PI / 180));
   }

   static Matrix4 makeZRotation(const double ang)
   {
      return makeZRotation(std::cos(ang * CS175_PI / 180), std::sin(ang 
      * CS175_PI / 180));
   }
    ...
    const double zNear, const double zFar)
   {
      ...
      const double f = std::abs(std::sin(ang)) < CS175_EPS ? 0 : 1 
      / std::tan(ang);
      ...
      if (std::abs(zFar - zNear) > CS175_EPS)
      { // 3rd row
