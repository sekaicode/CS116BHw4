...
_refraction = 1;}
		Material(const Point& a, const Pambientoint& d, const Point& s, const Point& t, GLdouble r)
...
Point lightPosition(0.0, 0.0, 0.0); 
/* although the ray tracer actually supports
	...
	default value to a value on the che light
...
void init()
{
	...
	while(tmp != "done")
	{
		cout << "Enter your object  (light, tetrahedron, cube, sphere, cylinder, cone):\n";
		cin >> tmp;
		if (tmp == "light")
		{
		...
		if (tmp == "cylinder")
		{
				...
				cin >> tmp;
				//tmp = "a7";
				Cylinder *cylinder = new Cylinder(convertStringCoordinate(tmp), SQUARE_EDGE_SIZE);	
				scene.addRayObject(cylinder);
		}
		if (tmp == "cone")
		{
				cout << "Please enter the position of the cube:\n";
				cin >> tmp;
				//tmp = "a7";
				Cone *cone = new Cone(convertStringCoordinate(tmp), SQUARE_EDGE_SIZE);	
				scene.addRayObject(cone);
		}
	}
